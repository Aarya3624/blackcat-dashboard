...
//   useEffect(() => {
//     socketRef.current = io("http://192.168.0.107:5000/video", {
...
//     axios.get("http://192.168.0.107:5000/halls").then((response) => {
...
//   ) => {
//     axios
//       .post("http://192.168.0.107:5000/add_camera", {
//         camera_id: cameraId,
...
//     axios
//       .post("http://192.168.0.107:5000/remove_camera", {
//         camera_id: cameraId,
//         hall_id: hallId,
//       })
//       .then((response) => {
...
const PeopleCounter = () => {
  ...
  const THRESHOLD: number = 20; // Define the threshold value
  ...
  useEffect(() => {
    ...
    socketRef.current.on("count", (data: any) => {
      ...
      Object.keys(count).forEach((hall) => {
        Object.values(count[hall].inside || {}).forEach((value) => {
          const insideCount = value as number;
          if (insideCount > THRESHOLD) {
            alert(`Alert: The count in hall ${hall} exceeds ${THRESHOLD}!`);
          }
        });
      });
    });
              ...
              <TableBody>
                {Object.keys(halls[hall].entered).map((cameraId) => (
                  <TableRow key={cameraId}>
                    ...
                    <TableCell
                      className={
                        halls[hall].entered[cameraId] > THRESHOLD
                    ...
                    <TableCell
                      className={
                        halls[hall].exited[cameraId] > THRESHOLD
                          ? "text-red-500 font-bold"
                          : ""
                      }
                    >
                    ...
                    <TableCell
                      className={
                        halls[hall].inside[cameraId] > THRESHOLD
                          ? "text-red-500 font-bold"
                          : ""
                      }
                    >
